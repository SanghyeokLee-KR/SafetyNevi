...
async function drawZone(zone, alert) {
    if (!zone || zoneGraphics.has(zone.id))
        return;
    const graphics = [];
...
function showDisasterAlert(zone) {
    if (!zone || alertedIds.has(zone.id) || isModalShowing)
        return;
    isModalShowing = true;
...
function getDisasterStyle(type) {
    ...
    if (t.match(/fire|missile|heat|화재/))
        return { fill: '#FF0000', stroke: '#FF0000' };
    if (t.match(/water|rain|flood|tsunami|호우/))
        return { fill: '#0000FF', stroke: '#0000FF' };
    if (t.match(/quake|지진/))
        return { fill: '#8B4513', stroke: '#D2691E' };
    if (t.match(/snow|cold|대설/))
        return { fill: '#B0C4DE', stroke: '#778899' };
    if (t.match(/dust|황사/))
        return { fill: '#FFD700', stroke: '#DAA520' };
...
function getDisasterMarkerImage(type) {
    ...
    for (const key in disasterMarkerImages) {
        if (t.includes(key))
            return disasterMarkerImages[key];
    }
    return disasterMarkerImages.default;
}
// 원형 구역 그리기
...
async function drawPolygonZone(areaName, style, markerImg, graphics) {
    try {
        ...
        if (count > 0)
            drawMarker(latSum / count, lngSum / count, markerImg, graphics);
    }
    catch (e) {
