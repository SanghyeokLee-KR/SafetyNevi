...
export function setupMyPlaceLogic() {
    ...
    if (btnHome)
        btnHome.addEventListener('click', () => openAddressSearch('HOME'));
    if (btnCompany)
        btnCompany.addEventListener('click', () => openAddressSearch('COMPANY'));
    ...
    subTabs.forEach(tab => {
        tab.addEventListener('click', () => {
            ...
            Object.values(contents).forEach(el => { if (el)
...
function renderMyPlacesAndMarkers() {
    ...
    if (homeText && home) {
        ...
        document.getElementById('btn-set-home').parentElement.onclick = (e) => {
            if (!e.target.classList.contains('kb-place-action'))
                moveToPlace(home.latitude, home.longitude);
        };
    }
    ...
    if (compText && company) {
        ...
        document.getElementById('btn-set-company').parentElement.onclick = (e) => {
            if (!e.target.classList.contains('kb-place-action'))
                moveToPlace(company.latitude, company.longitude);
        };
    }
            ...
            favorites.forEach(fav => {
                ...
                item.addEventListener('click', (e) => {
                    if (!e.target.classList.contains('btn-delete-fav'))
                        moveToPlace(fav.latitude, fav.longitude);
                });
