...
export function setupTabNavigation() {
    ...
    if (!routeFinder || !generalContent || !myPage)
        return;
    tabButtons.forEach(button => {
        button.addEventListener('click', function () {
            ...
            if (targetTab === 'route')
                routeFinder.style.display = 'block';
...
export function setupDetailViewEvents() {
    ...
    if (backBtn) {
        backBtn.addEventListener('click', function () {
            ...
            if (generalTab)
                generalTab.classList.add('kb-active');
        });
    }
...
export function updateSidebar(data) {
    ...
    if (timeEl)
        timeEl.innerText = data.time || "정보 없음";
    const statusEl = document.getElementById('detail-status');
    let rawStatus = data.operatingStatus || '정보없음';
    if (data.type === 'police' || data.type === 'fire')
        rawStatus = '영업';
    ...
    if (routeBtn)
        routeBtn.parentNode.insertBefore(actionContainer, routeBtn);
    ...
    document.getElementById('btn-copy-addr').onclick = () => navigator.clipboard.writeText(data.address || "").then(() => showToast("주소가 복사되었습니다!"));
    if (routeBtn) {
