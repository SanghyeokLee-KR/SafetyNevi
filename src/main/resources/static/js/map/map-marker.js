...
export async function updateMarkers() {
    ...
    try {
        const requests = facilityTypes.map(type => fetch(`/api/facilities?type=${type}&${queryParams}`).then(res => res.json()));
        ...
        if (window.calculateSafetyScore)
            window.calculateSafetyScore(allFacilities);
    }
    catch (error) {
...
function getMarkerImage(facility) {
    ...
    if (status && (status.includes('휴업') || status.includes('일시중지')))
        return markerImages.resting;
    if (status && (status.includes('폐업') || status.includes('취소')))
        return markerImages.default;
    if (type === 'police')
        return markerImages.police;
    if (type === 'fire')
        return markerImages.fire;
    if (type === 'hospital')
        return markerImages.hospital;
    if (type === 'shelter') {
        if (capacity >= 1000)
            return markerImages.shelter_high;
        if (capacity >= 300)
            return markerImages.shelter_mid;
...
export function setupMapEventListeners() {
    ...
    const showReSearchBtn = () => { if (reSearchBtn)
