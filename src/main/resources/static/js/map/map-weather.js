...
function updateWeatherUI(data) {
    ...
    if (addrEl)
        addrEl.innerText = data.address || "주소정보 없음";
    const tempEl = document.querySelector('#current-temp');
    const temp = data.temp;
    if (tempEl)
        tempEl.innerText = (temp && temp !== 'N/A') ? `${temp}°` : '';
    const weatherStatusEl = document.querySelector('#weather-status');
    if (weatherStatusEl)
        weatherStatusEl.innerText = data.weatherStatus || "";
    ...
    if (weatherIconEl && data.weatherStatus) {
        ...
        if (status.includes('맑음'))
            iconSrc = 'sunny.png';
        else if (status.includes('구름'))
            iconSrc = 'cloudy.png';
        else if (status.includes('흐림'))
            iconSrc = 'overcast.png';
        else if (status.includes('비'))
            iconSrc = 'rain.png';
