...
document.addEventListener('DOMContentLoaded', () => {
    ...
    async function checkDuplicate(url, input, msgElem, successMsg, failMsg) {
        try {
            ...
            if (data.available)
                setStatus(input, msgElem, true, successMsg);
            else
                setStatus(input, msgElem, false, failMsg);
        }
        catch (err) {
            console.error(err);
        }
    ...
    const updateMeter = (val) => {
        ...
        if (val.length < 8) {
            ...
            pwBar.style.backgroundColor = '#dc3545';
        }
        else if (score < 3) {
            pwBar.style.width = '50%';
            pwBar.style.backgroundColor = '#ffc107';
        }
        else {
            pwBar.style.width = '100%';
            pwBar.style.backgroundColor = '#28a745';
        }
    };
    const checkPwMatch = () => {
