...
document.addEventListener('DOMContentLoaded', () => {
    ...
    infoForm?.addEventListener('submit', async (e) => {
        e.preventDefault();
        if (!REGEX.nickname.test(nickInput.value)) {
            nickInput.focus();
            return alert("닉네임을 확인해주세요.");
        }
        if (!REGEX.phone.test(phoneInput.value)) {
            phoneInput.focus();
    ...
    pwForm?.addEventListener('submit', async (e) => {
        ...
        if (!REGEX.password.test(newPw.value))
    ...
    boardList?.addEventListener('click', async (e) => {
        ...
        // 삭제
        const delBtn = targetEl.closest('.btn-delete-post');
        if (delBtn) {
            e.stopPropagation();
        ...
        // 상세보기
        const item = targetEl.closest('.kb-board-item');
        if (item) {
    ...
    function renderPostModal(data) {
        ...
        const commentsHtml = data.comments?.length
            ? data.comments.map((c) => `<li class="kb-post-comment-item">
